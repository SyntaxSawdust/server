...
class RecentPlayedTrack(NamedTuple):
    """A recently played track from the playlog, with the artists/album recorded at play time."""
...
class MusicController(MusicDatabaseSetupMixin, CoreController):
    ...
    async def recently_played_tracks(
        ...
        """
        Return recently played, fully played tracks with their recorded artists/album, newest first.
        ...
        """
        query = (
            f"SELECT item_id, provider, name, artists, album FROM {DB_TABLE_PLAYLOG} "
        ...
        return [
            RecentPlayedTrack(
                ...
                album=ItemMapping.from_dict(json_loads(db_row["album"]))
                if db_row["album"]
                else None,
            )
            for db_row in db_rows
